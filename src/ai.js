...
class AIHandler {
    ...
    initClients() {
        ...
        this.instantClients = allKeys.slice(0, 3).map(key => new Groq({ apiKey: key, timeout: 25000, maxRetries: 2 }));
        this.thinkingClients = allKeys.slice(3, 5).map(key => new Groq({ apiKey: key, timeout: 25000, maxRetries: 2 }));
    ...
    getRandomClient() {
        ...
        if (!pool.length) { Logger.warn('⚠️ Instant pool trống!'); return this.thinkingClients[0] || null; }
        return pool[Math.floor(Math.random() * pool.length)];
    }

    getThinkingClient() {
        const pool = this.thinkingClients;
        if (!pool.length) { Logger.warn('⚠️ Thinking pool trống, fallback về Instant pool'); return this.getRandomClient(); }
        return pool[Math.floor(Math.random() * pool.length)];
    }
    ...
    async process(userId, question, model = 'instant', context = '') {
        ...
        if (!cdCheck.allowed) return `⏰ Chờ ${cdCheck.wait}s trước khi dùng ${model === 'thinking' ? 'Thinking' : 'Instant'}.`;

        const quotaCheck = await this.checkQuota(userId, model);
        if (!quotaCheck.allowed) {
            if (quotaCheck.reason === 'not_logged_in') return `🔒 Bạn chưa đăng nhập. Gõ \`${Config.PREFIX}login\` để đăng nhập.`;
            if (quotaCheck.reason === 'quota_exceeded') {
                if (model === 'instant') return `📊 Bạn đã dùng hết ${Config.INSTANT_DAILY_LIMIT} lượt Instant hôm nay.\n💡 Dùng \`${Config.PREFIX}model thinking\` để chuyển sang Thinking (${Config.THINKING_DAILY_LIMIT} lượt/ngày).\n🔄 Reset lúc 7h sáng mai.`;
        ...
        try {
            ...
            // Lưu Q&A gộp 1 document duy nhất
            const qaData = { question: question.slice(0, 500), answer: safeReply, model };
            if (isPrivate) {
                await Firebase.addPrivateHistory(userId, qaData)
                    .catch(e => Logger.error('addPrivateHistory error:', e.message));
            } else {
                await Firebase.addHistory(userId, qaData)
                    .catch(e => Logger.error('addHistory error:', e.message));
            }
    ...
    async ask(userId, question, model = 'instant') { return this.process(userId, question, model, 'instant'); }
    async search(userId, query, model = 'instant') { return this.process(userId, `🔍 Tìm kiếm: ${query}`, model, 'search'); }
}
