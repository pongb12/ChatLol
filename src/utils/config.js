...
class Config {
    ...
    validate() {
        ...
        if (this.GROQ_API_KEYS.length < 3) Logger.warn(`⚠️ Instant pool cần 3 keys, hiện có ${Math.min(this.GROQ_API_KEYS.length, 3)}`);
        if (this.GROQ_API_KEYS.length < 4) Logger.warn('⚠️ Thinking pool cần key 4,5 — sẽ fallback về Instant pool');
